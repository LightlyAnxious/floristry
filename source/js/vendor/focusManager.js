...
function focusFirstInElement(element) {
  let focusElement = findFirstFocusableElement(element) || element;
  focus(focusElement);
}

function focusLastInElement(element) {
  let focusElement = findLastFocusableElement(element) || element;
  focus(focusElement);
}

// State is kept is these letiables.
...
function releaseModalFocus(focusElement) {
  let eventListenerContext = state.eventListenerContext;
  let eventListenerArguments = state.eventListenerArguments;

  if (eventListenerContext && eventListenerArguments) {
    eventListenerContext.removeEventListener.apply(
...
function captureModalFocus(modal, focusElement, backgroundElement) {
  ...
  let eventListenerContext = backgroundElement || document;
  let eventListenerArguments = ['focus', focusCallback, true];
  ...
  eventListenerContext.addEventListener.apply(
    eventListenerContext,
    eventListenerArguments
  );
}
...
export default function focusManagerFactory() {
  return focusManager;
}
